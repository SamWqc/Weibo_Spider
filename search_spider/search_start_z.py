...
def fetch_weibo_data(keyword, start_time, end_time, page_id):
    ...
    for card in all_contents:
        if (card.get('mid') != None):  # 如果微博ID不为空则开始抓取
            ...
            wb_id = str(card.select('.from')[-1].select_one('a').get('href')).split('/')[-1].split('?')[0]  # 微博ID
            ...
            '''转发微博的爬取'''
            card_zhuanfa = card.select_one('.card-comment')
            if card_zhuanfa != None:
                # print('--------------------转发微博信息-------------------------')
                zhuan_username = card_zhuanfa.select_one('.name').get('nick-name')
                zhuan_userid = card_zhuanfa.select_one('.name').get('href')[14:]
                zhuan_wb_content = filter_emoji(card_zhuanfa.select_one('.txt').text.strip())

                zhuan_wb_create = card_zhuanfa.select_one('.from').select_one('a').text.strip()  # 微博创建时间
                zhuan_wb_url = 'https:' + str(card_zhuanfa.select_one('.from').select_one('a').get('href'))  # 微博来源URL
                zhuan_wb_id = \
                str(card_zhuanfa.select_one('.from').select_one('a').get('href')).split('/')[-1].split('?')[0]  # 微博来源id
                zhuan_wb_createtime = time_process(zhuan_wb_create)

                zhuan_wb_forwardnum = num_process(str(card_zhuanfa.select_one('.func').select('li')[0].text))  # 微博转发数
                zhuan_wb_commentnum = num_process(
                    str(card_zhuanfa.select_one('.func').select('li')[1].text).strip())  # 微博评论数
                zhuan_wb_like = str(card_zhuanfa.select_one('.func').select_one('em').text)  # 微博点赞数
                if (zhuan_wb_like == ''):  # 点赞数的处理
                    zhuan_wb_likenum = '0'
                else:
                    zhuan_wb_likenum = zhuan_wb_like

                blog_zhuan = {'wb_id': zhuan_wb_id,  # 生成一条微博记录的列表
                              'wb_username': zhuan_username,
                              'wb_userid': zhuan_userid,
                              'wb_content': zhuan_wb_content,
                              'wb_createtime': zhuan_wb_createtime,
                              'wb_forwardnum': zhuan_wb_forwardnum,
                              'wb_commentnum': zhuan_wb_commentnum,
                              'wb_likenum': zhuan_wb_likenum,
                              'wb_url': zhuan_wb_url,
                              'zhuan_wb_id': wb_id
                              }
                mblog.append(blog_zhuan)
...
def fetch_pages(keyword, start_time, end_time):
    ...
    sql = "insert into keyword_weibo(wb_id,wb_username,wb_userid,wb_content,wb_createtime,wb_forwardnum,wb_commentnum,wb_likenum,wb_url,zhuan_wb_id,keyword) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    mh.open();
    for i in range(len(mblogs)):
        ...
        mblogs[i]['wb_url'], mblogs[i]['zhuan_wb_id'], keyword))
    mh.tijiao();
    mh.close()
